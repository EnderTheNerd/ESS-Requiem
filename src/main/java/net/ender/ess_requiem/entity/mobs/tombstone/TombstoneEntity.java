...
public class TombstoneEntity extends UniqueAbstractSpellCastingMob implements IMagicSummon, IAnimatedAttacker {
    ...
    public TombstoneEntity(Level level, LivingEntity owner) {
        this(GGEntityRegistry.TOMBSTONE.get(), level);
        setSummoner(owner);
    }


    public TombstoneEntity(EntityType<? extends AbstractSpellCastingMob> entityType, Level world) {
    ...
    private final AnimationController<TombstoneEntity> animationController = new AnimationController<>(this, "controller", 0, this::predicate);
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if (getMagicData().isCasting()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("cast", Animation.LoopType.PLAY_ONCE));
